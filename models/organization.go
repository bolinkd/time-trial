...
func (o *Organization) Groups(mods ...qm.QueryMod) groupQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"Group\".\"organization_id\"=?", o.ID),
	)
...
// Rowers retrieves all the rower's Rowers with an executor.
func (o *Organization) Rowers(mods ...qm.QueryMod) rowerQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"rower\".\"organization_id\"=?", o.ID),
	)
...
func (organizationL) LoadGroups(e boil.Executor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`Group`), qm.WhereIn(`organization_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadRowers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadRowers(e boil.Executor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(qm.From(`rower`), qm.WhereIn(`organization_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.OrganizationID) {
				local.R.Rowers = append(local.R.Rowers, foreign)
				if foreign.R == nil {
					foreign.R = &rowerR{}
				}
				foreign.R.Organization = local
...
func (o *Organization) AddGroupsG(insert bool, related ...*Group) error {
...
// Appends related to o.R.Groups.
// Sets related.R.Organization appropriately.
func (o *Organization) AddGroups(exec boil.Executor, insert bool, related ...*Group) error {
	var err error
...
// Sets o.R.Organization's Groups accordingly.
// Replaces o.R.Groups with related.
// Sets related.R.Organization's Groups accordingly.
// Uses the global database handle.
func (o *Organization) SetGroupsG(insert bool, related ...*Group) error {
...
// Replaces o.R.Groups with related.
// Sets related.R.Organization's Groups accordingly.
func (o *Organization) SetGroups(exec boil.Executor, insert bool, related ...*Group) error {
	query := "update \"Group\" set \"organization_id\" = null where \"organization_id\" = $1"
...
// Uses the global database handle.
func (o *Organization) RemoveGroupsG(related ...*Group) error {
...
// Removes related items from R.Groups (uses pointer comparison, removal does not keep order)
// Sets related.R.Organization.
func (o *Organization) RemoveGroups(exec boil.Executor, related ...*Group) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.OrganizationID, nil)
		if rel.R != nil {
...
// AddRowersG adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.Rowers.
// Sets related.R.Organization appropriately.
// Uses the global database handle.
func (o *Organization) AddRowersG(insert bool, related ...*Rower) error {
	return o.AddRowers(boil.GetDB(), insert, related...)
}

// AddRowers adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.Rowers.
// Sets related.R.Organization appropriately.
func (o *Organization) AddRowers(exec boil.Executor, insert bool, related ...*Rower) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.OrganizationID, o.ID)
	...
	if o.R == nil {
		...
		o.R.Rowers = append(o.R.Rowers, related...)
	}
...
// SetRowersG removes all previously related items of the
// organization replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Organization's Rowers accordingly.
// Replaces o.R.Rowers with related.
// Sets related.R.Organization's Rowers accordingly.
// Uses the global database handle.
func (o *Organization) SetRowersG(insert bool, related ...*Rower) error {
	return o.SetRowers(boil.GetDB(), insert, related...)
}

// SetRowers removes all previously related items of the
// organization replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Organization's Rowers accordingly.
// Replaces o.R.Rowers with related.
// Sets related.R.Organization's Rowers accordingly.
func (o *Organization) SetRowers(exec boil.Executor, insert bool, related ...*Rower) error {
	query := "update \"rower\" set \"organization_id\" = null where \"organization_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	return o.AddRowers(exec, insert, related...)
}

// RemoveRowersG relationships from objects passed in.
// Removes related items from R.Rowers (uses pointer comparison, removal does not keep order)
// Sets related.R.Organization.
// Uses the global database handle.
func (o *Organization) RemoveRowersG(related ...*Rower) error {
	return o.RemoveRowers(boil.GetDB(), related...)
}

// RemoveRowers relationships from objects passed in.
// Removes related items from R.Rowers (uses pointer comparison, removal does not keep order)
// Sets related.R.Organization.
func (o *Organization) RemoveRowers(exec boil.Executor, related ...*Rower) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.OrganizationID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.Rowers {
			...
			ln := len(o.R.Rowers)
			if ln > 1 && i < ln-1 {
				o.R.Rowers[i] = o.R.Rowers[ln-1]
			}
			o.R.Rowers = o.R.Rowers[:ln-1]
			break
		}
	}
